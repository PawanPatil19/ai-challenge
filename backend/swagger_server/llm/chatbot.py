...
class ChatBot:
    ...
    def preload_conversation(self, conversation_history=None):
        if isinstance(conversation_history, list):
            ...
            self._conversation_history = [
                self._construct_message('From now on, return the putput as a JSON object that can be loaded in Python with the key as \'text\'.'),
                self._construct_message('"Sure, I can return the output as a regular JSON object with the key as "text". Here is an example', role='model')
                ]
